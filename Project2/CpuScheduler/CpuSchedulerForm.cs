...
// 
// References
// Claude AI (Claude Sonnet 4.6) by Anthropic (https://claude.ai)
// Used to debug code and as a general reference
...
4. Compare algorithm behaviors and outcomes";
        ...
        private List<SchedulingResult> RunSRTFAlgorithm(List<ProcessData> processes)
        {
            ...
            var processResults = new Dictionary<string, SchedulingResult>();
            var remainingBurstTimes = new Dictionary<string, int>();
            var switched = false; // Variable to track if process was preempted
            ...
            {
                var availableProcesses = remainingProcesses.Where(p => p.ArrivalTime <= currentTime).ToList();
                if (availableProcesses.Count == 0) // Check for arrived processes
                {
                    currentTime = remainingProcesses.Min(p => p.ArrivalTime);
                    continue;
                }
                var nextProcess = availableProcesses.OrderBy(p => remainingBurstTimes[p.ProcessID]).ThenBy(p => p.ArrivalTime).First(); // Choose initial process with shortest remaining burst time
                while(remainingBurstTimes[nextProcess.ProcessID] > 0) //Loop while there is still remaining time for current process
                {
                    if (processResults[nextProcess.ProcessID].StartTime == -1) // Assign start time if not yet assigned
                    {
                        processResults[nextProcess.ProcessID].StartTime = currentTime;
                    }
                    currentTime += 1; // Incremement time
                    remainingBurstTimes[nextProcess.ProcessID] -= 1; // Decremenent remaining time for the current process

                    availableProcesses = remainingProcesses.Where(p => p.ArrivalTime <= currentTime).ToList(); // Update processes to current time
                    var shortestProcess = availableProcesses.OrderBy(p => remainingBurstTimes[p.ProcessID]).ThenBy(p => p.ArrivalTime).First(); // Check if any other process now has a shorter remaining burst time
                    if (remainingBurstTimes[nextProcess.ProcessID] > remainingBurstTimes[shortestProcess.ProcessID])
                    {
                        nextProcess = shortestProcess; // Switch nextProcess if there is a process with a shorter remaining time
                        switched = true;
                        break; // Exit and go back to the top of the loop
                    }
                }
                // Calculate finish, waiting, and turnaround time, if a process was finished
                if (!switched) 
                {
                    processResults[nextProcess.ProcessID].FinishTime = currentTime;
                    processResults[nextProcess.ProcessID].WaitingTime = processResults[nextProcess.ProcessID].StartTime - processResults[nextProcess.ProcessID].ArrivalTime;
                    processResults[nextProcess.ProcessID].TurnaroundTime = processResults[nextProcess.ProcessID].FinishTime - processResults[nextProcess.ProcessID].ArrivalTime;
                
                    remainingProcesses.Remove(nextProcess); // Remove finished process
                }
                switched = false;
            }

            return processResults.Values.OrderBy(r => r.StartTime).ToList(); // Return results
        }
        ...
        private List<SchedulingResult> RunHRRNAlgorithm(List<ProcessData> processes)
        {
            ...
            while (remainingProcesses.Count > 0)
            {
                ...
                // Pick the process with the highest response ratio ((waiting time + burst time) / burst time)
                var nextProcess = availableProcesses.OrderByDescending(p => ((double)((currentTime - p.ArrivalTime) + p.BurstTime)/ p.BurstTime)).ThenBy(p => p.ArrivalTime).First();
        ...
        private void DisplaySchedulingResults(List<SchedulingResult> results, string algorithmName)
        {
            ...
            // CPU Utilization
            var cpuUtilization = ((double)results.Sum(r => r.BurstTime) /  results.Max(r => r.FinishTime) * 100);

            // Throughput
            var throughput = (double)results.Count() / results.Max(r => r.FinishTime);
            ...
            summaryItem.SubItems.Add($"Throughput: {throughput:F1} processes/second");
